// The Balise Worker under workerd, against a real local D1: the ROUTES and the contract
// envelopes.
//
// This file ran to 858 lines and was three concerns by then, so A31 split it in three. The other
// two are its siblings and neither is optional reading:
//
//   tests/local-d1-rows.test.mjs   how MUCH the database reads: the A4 rows_read budgets, keyset
//                                  paging, and the five index-selection cases that make phase 1
//                                  tenancy cost nothing.
//   tests/local-d1-plans.test.mjs  HOW it answers: every query plan pinned by index name, with
//                                  each transcribed statement proved against the route that
//                                  runs it (A20, A28).
//
// The split is by concern rather than by line count, and each file seeds its own database: a
// budget assertion and a contract assertion want different fixtures, and the old file's tenant
// fixture existed for the budgets alone.
//
// What is left here is C1 to C4: ingest, the duplicate guard, origins, operator authentication
// and the transition table, plus the envelope promise that nothing is ever a 500. It still runs
// `wrangler dev` and talks to it over HTTP, which is the only way to exercise the two things node
// cannot fake:
//
//   1. `crypto.subtle.timingSafeEqual`, a Workers runtime extension that does not exist in node,
//      and therefore the entire C3 comparison and lockout.
//   2. D1 itself: the unique index that is the duplicate guard, and the guarded UPDATE that
//      enforces C4.
//
// WHAT THIS FILE CANNOT PROVE, and neither can any other local test:
//   - Every D1 limit and quota. Local D1 is the same workerd binary over a real SQLite file, so
//     SQL behaviour reproduces, but no limit and no quota does (cloudflare/workers-sdk#6347: a
//     migration that passes --local fails --remote with SQLITE_TOOBIG).
...
//     (https://developers.cloudflare.com/turnstile/troubleshooting/testing/). A real sitekey, a
//     real browser and a real token are never involved.
//
// It also makes real network calls to challenges.cloudflare.com. That is deliberate: a stubbed
// siteverify would prove nothing about the code path that matters.
...
// Process control and the request helper live in ./harness.mjs, which every workerd suite
// imports. Two copies of "spawn wrangler dev and wait for /health" would drift.
...
before(async () => {
  // A fresh database every run. The per-site read-back below only means something against a
